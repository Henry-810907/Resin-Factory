...
type Props = { dict: Dictionary["whatsapp"] };

const PHONE = "8613682692148";
const DISPLAY = "+86 136 8269 2148";

export default function WhatsAppFloat({ dict }: Props) {
  const href = `https://wa.me/${PHONE}?text=${encodeURIComponent(dict.prefill)}`;
  return (
    <a
      href={href}
      target="_blank"
      ...
      className="group fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex items-center gap-3 bg-[#25D366] hover:bg-[#1EBE5D] rounded-full shadow-lg transition transform hover:scale-105 px-4 py-3 sm:px-5 sm:py-3.5"
    >
      <svg width="24" height="24" viewBox="0 0 32 32" fill="currentColor" className="sm:w-6 sm:h-6 shrink-0" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
