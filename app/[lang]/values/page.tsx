  ...
  return (
    <main>
      ...
      <section className="bg-white py-14 md:py-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-xs uppercase tracking-[0.2em] text-brand-orange font-bold mb-3">{v.principlesKicker}</p>
            <h2 className="text-3xl md:text-4xl font-bold text-brand-dark tracking-tight">{v.principlesTitle}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {v.principles.map((p) => (
              <div key={p.title} className="bg-white rounded-xl p-7 shadow-sm border border-slate-200 hover:shadow-md hover:-translate-y-1 transition">
      ...
      <section className="bg-brand-bgAlt py-14 md:py-20 border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-xs uppercase tracking-[0.2em] text-brand-orange font-bold mb-3">{v.peopleKicker}</p>
            <h2 className="text-3xl md:text-4xl font-bold text-brand-dark leading-tight mb-4 tracking-tight">{v.peopleTitle}</h2>
            <p className="text-base md:text-lg text-slate-600 max-w-3xl mx-auto leading-relaxed">{v.peopleIntro}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-14">
            {v.commitments.map((c, i) => (
              <div key={c.title} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition overflow-hidden flex flex-col">
                ...
                <div className="p-6">
                  <h3 className="text-lg font-bold text-brand-dark mb-2">{c.title}</h3>
                  <p className="text-slate-600 leading-relaxed text-[15px]">{c.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-brand-dark text-white rounded-2xl p-10 md:p-14 grid grid-cols-1 md:grid-cols-5 gap-10 items-center">
            <div className="md:col-span-3 space-y-5">
              <h3 className="text-3xl md:text-4xl font-bold leading-tight tracking-tight">{v.manifestoTitle}</h3>
              <p className="text-slate-300 text-lg leading-relaxed">{v.manifestoP1}</p>
              <p className="text-slate-400 text-base leading-relaxed">{v.manifestoP2}</p>
            </div>
      ...
      <section className="bg-white py-14 md:py-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-xs uppercase tracking-[0.2em] text-brand-orange font-bold mb-3">{v.teamKicker}</p>
            <h2 className="text-3xl md:text-4xl font-bold text-brand-dark tracking-tight mb-3">{v.teamTitle}</h2>
            <p className="text-slate-600 max-w-2xl mx-auto text-base md:text-lg">{v.teamSubtitle}</p>
          </div>
          <p className="text-center text-sm text-slate-500 italic mt-10">{v.teamCaption}</p>
        </div>
      </section>
